...
def main():
    ...
    if not manifest_path.exists():
        raise FileNotFoundError(
            "Le fichier generated_samples_manifest.csv est introuvable. "
            "Lance d'abord : python src/sample_latent_diffusion.py"
        )
    ...
    for epoch in range(1, CLASSIFIER_EPOCHS + 1):
        ...
        metrics["scenario"] = "S2_real_plus_ldm"
        ...
        if metrics["f1_score"] > best_f1:
            ...
            torch.save(
                ...
                CLASSIFIER_DIR / "classifier_s2_best.pt",
            )

        print(
            f"S2 epoch {epoch}/{CLASSIFIER_EPOCHS} | "
    ...
    df.to_csv(METRICS_DIR / "classifier_s2_metrics.csv", index=False)
    ...
    confusion.to_csv(CLASSIF_DIR / "confusion_matrix_s2.csv", index=False)

    print("Entraînement classifieur S2 terminé.")
